package com.example.petstorerest.controller;

import com.example.petstorerest.exception.DataNotFoundException;
import com.example.petstorerest.exception.InvalidIdentifierException;
import com.example.petstorerest.model.Pet;
import com.example.petstorerest.service.PetService;
...
public class PetController {
    ...
    private final static String noFoundMessage = "Pet not found";
    @ApiOperation(value = "Get User by id ", response = Pet.class)
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Success"),
            @ApiResponse(code = 400, message = "Invalid ID supplied"),
            @ApiResponse(code = 404, message = noFoundMessage) })
    @GetMapping(value = "/{id}", produces = APPLICATION_JSON_VALUE)
    public Pet getPetById(@PathVariable Long id) {

        if (id == null) {
            throw new InvalidIdentifierException("Invalid ID supplied");
        }

        Pet pet = petService.getPetById(id);
        if (pet == null) {
            throw new DataNotFoundException(noFoundMessage);
        }
    ...
    public List<Pet> findPetsByStatus(@RequestParam("status") String status) {
        ...
        statues.forEach(s -> {
            ...
            {
                throw new InvalidIdentifierException("Invalid status value");
            }
        });
